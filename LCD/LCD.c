...
static void LCD_FSMCConfig(void)
{
	...
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 50;//1;
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 10;
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 50;//1;
	FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 5;
...
void LCD_DrawRectangle(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t borderWidth, uint16_t color){
	uint16_t i;
	...
	for(i=0;i<borderWidth;i++){
		LCD_DrawLine(x,y+i,x_end,y+i,color);
		LCD_DrawLine(x,y_end-i,x_end,y_end-i,color);
		LCD_DrawLine(x+i,y,x+i,y_end,color);
		LCD_DrawLine(x_end-i,y,x_end-i,y_end,color);
	}
}
...
* Attention		 : None
*******************************************************************************/
void LCD_PrintText(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
